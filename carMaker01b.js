...
const titles = ["id", "firstName", "lastName", "carMake", "carModel", "carModelYear", "carSerial", "carCost"]
...
for (i=0; i < rawData.length; i++) {
    if (carMaker.indexOf(rawData[i][3]) == -1)
        carMaker.push(rawData[i][3])
        carModels.push([])
}


for (i = 0; i < carMaker.length; i++) {
    for (j = 0; j < rawData.length; j++) {
        if (carMaker[i] == rawData[j][3]) {
            carModels[i].push(rawData[j])
        }
    }
